...
export default function Page() {
  ...
  return (
    <main className="min-h-screen bg-soft pb-[calc(6rem+env(safe-area-inset-bottom))] pt-[env(safe-area-inset-top)] text-ink dark:bg-slate-950 dark:text-slate-100 lg:h-screen lg:overflow-hidden lg:pb-0 lg:pt-0">
      <div className="flex min-h-screen w-full gap-0 lg:h-screen">
        ...
        <section className="min-w-0 flex-1 px-4 py-4 sm:px-6 lg:h-screen lg:overflow-y-auto lg:px-7">
          <header className="sticky top-0 z-30 mb-5 flex flex-col gap-4 rounded-3xl border border-line bg-white/95 p-4 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 dark:shadow-none sm:flex-row sm:items-center sm:justify-between lg:static">
            <div className="min-w-0">
              <div className="mb-3 flex items-center gap-2 lg:hidden">
                <img src="/summer-stash-logo.png" alt="Summer Stash" className="h-10 w-10 rounded-xl object-contain" />
                <span className="text-sm font-black uppercase tracking-wide text-[#24476f] dark:text-slate-100">Summer Stash</span>
              </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-white/95 px-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] pt-2 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/95 lg:hidden">
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex items-end bg-slate-950/45 p-3 lg:hidden" onClick={() => setMobileMenuOpen(false)}>
          <div
            className="w-full rounded-t-3xl border border-line bg-white p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-soft dark:border-slate-800 dark:bg-slate-900"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between gap-3">
              <BrandLogo />
              <button className="icon-button" title="Cerrar menu" onClick={() => setMobileMenuOpen(false)}>
                <X size={18} />
              </button>
            </div>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-14 items-center gap-3 rounded-2xl border px-3 py-3 text-left text-sm font-bold ${
                      active
                        ? "border-blue-600 bg-blue-600 text-white"
                        : "border-line bg-white text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200"
                    }`}
                  >
                    <Icon size={18} />
                    <span className="truncate">{item.label}</span>
